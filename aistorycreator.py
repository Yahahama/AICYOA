...
def createStory(id='', prevGenStories=[]):
    global currentStory
    currentDepth = len(id)
    if currentDepth > finalDepth: return  # Base case of recursive function
    if currentDepth > 0: currentStory = prevGenStories[-1][int(id[-1])]
    newNode = {}
    newNode.update({"id": id})
    newNode.update({"text": currentStory})  # Each layer uses the story generated by the previous layer
    if currentDepth != finalDepth:  # Don't generate additional choices and stories at the final layer, just use a previous story
        if currentDepth == finalDepth - 1:  # If we're at second-to-last depth, prompt attempts to generate conclusions rather than continuations
